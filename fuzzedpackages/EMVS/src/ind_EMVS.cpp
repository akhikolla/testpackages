...
// [[Rcpp::export(".ind_EMVS")]]
SEXP ind_EMVS(SEXP Y_,
	  SEXP X_,
	  SEXP v0s_,
	  SEXP v1_,
	  SEXP type_,
  ...
  // Begin EMVS regularization
    ...
    for(int i = 0; i < L; ++i) {
      ...
      while(eps > epsilon) {
        ...
        E_step = ind_E_beta_binom(beta_k, v0, v1, theta_k, temperature);
        inv_var = E_step.col(0);
        post = E_step.col(1);
        
        // ******** M-step ************ //

        beta_k = ind_M_beta(XtY, X, Y, XtX, inv_var, sigma_k);
        sigma_k = ind_M_sigma(Y,X,beta_k, 1, 1);
