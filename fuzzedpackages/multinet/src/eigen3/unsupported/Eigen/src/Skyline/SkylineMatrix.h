    ...
    : public SkylineMatrixBase<SkylineMatrix<_Scalar, _Options> >
{
  ...
  public:
    ...
    inline Index
    upperNonZeros(Index j) const
    {
        return m_colStartIndex[j + 1] - m_colStartIndex[j];
    }

    inline Index
    lowerNonZeros(Index j) const
    {
    ...
    inline Scalar
    coeffLower(Index row, Index col) const
    {
        const Index outer = IsRowMajor ? row : col;
        const Index inner = IsRowMajor ? col : row;
    ...
    inline Scalar
    coeffUpper(Index row, Index col) const
    {
        const Index outer = IsRowMajor ? row : col;
        const Index inner = IsRowMajor ? col : row;
    ...
    inline Scalar&
    coeffRefLower(Index row, Index col)
    {
        const Index outer = IsRowMajor ? row : col;
        const Index inner = IsRowMajor ? col : row;
    ...
    inline bool
    coeffExistLower(Index row, Index col)
    {
        const Index outer = IsRowMajor ? row : col;
        const Index inner = IsRowMajor ? col : row;
    ...
    inline Scalar&
    coeffRefUpper(Index row, Index col)
    {
        const Index outer = IsRowMajor ? row : col;
        const Index inner = IsRowMajor ? col : row;
    ...
    inline bool
    coeffExistUpper(Index row, Index col)
    {
        const Index outer = IsRowMajor ? row : col;
        const Index inner = IsRowMajor ? col : row;
  ...
  public:
    ...
    inline void
    reserve(Index reserveSize, Index reserveUpperSize, Index reserveLowerSize)
    {
    ...
    operator=(const SkylineMatrix & other)
    {
    ...
    friend std::ostream &
    operator <<(std::ostream & s, const SkylineMatrix & m)
    {
