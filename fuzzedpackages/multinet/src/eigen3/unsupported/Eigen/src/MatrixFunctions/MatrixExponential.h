...
struct MatrixExponentialScalingOp
{
    ...
    operator() (const RealScalar& x) const
    {
        using std::ldexp;
     ...
     * \param[in,out] x  The scalar to be scaled, becoming \f$ 2^{-s} x \f$.
     */
    inline const ComplexScalar
    operator() (const ComplexScalar& x) const
    {
        using std::ldexp;
...
struct matrix_exp_computeUV
{
    /** \brief Compute Pad&eacute; approximant to the exponential.
      ...
      */
    static void
    run(const MatrixType& arg, MatrixType& U, MatrixType& V, int& squarings);
};
...
struct matrix_exp_computeUV<MatrixType, float>
{
    ...
    static void
    run(const ArgType& arg, MatrixType& U, MatrixType& V, int& squarings)
    {
        using std::frexp;
        using std::pow;
...
void
matrix_exp_compute(const ArgType& arg, ResultType &result)
{
