...
class MatrixPowerAtomic : internal::noncopyable
{
  private:
    ...
    void
    computePade(int degree, const MatrixType& IminusT, ResultType& res) const;
    void
    compute2x2(ResultType& res, RealScalar p) const;
    void
    computeBig(ResultType& res) const;
    static int
    getPadeDegree(float normIminusT);
    static int
    getPadeDegree(double normIminusT);
    static int
    getPadeDegree(long double normIminusT);
    static ComplexScalar
    computeSuperDiag(const ComplexScalar&, const ComplexScalar&, RealScalar p);
    static RealScalar
    computeSuperDiag(RealScalar, RealScalar, RealScalar p);
...
class MatrixPower : internal::noncopyable
{
  ...
  public:
    ...
    template<typename ResultType>
    void
    compute(ResultType& res, RealScalar p);
  ...
  private:
     ...
     */
    void
    split(RealScalar& p, RealScalar& intpart);
    ...
    template<typename ResultType>
    void
    computeIntPower(ResultType& res, RealScalar p);

    template<typename ResultType>
    void
    computeFracPower(ResultType& res, RealScalar p);
...
template<int Rows, int Cols, int Options, int MaxRows, int MaxCols>
inline void
MatrixPower<MatrixType>::revertSchur(
    Matrix<ComplexScalar, Rows, Cols, Options, MaxRows, MaxCols>& res,
    const ComplexMatrix& T,
    const ComplexMatrix& U)
{
