...
struct TensorEvaluator<const TensorFFTOp<FFT, ArgType, FFTResultType, FFTDir>, Device>
{
    ...
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE
    TensorEvaluator(const XprType& op, const Device& device) : m_fft(op.fft()), m_impl(op.expression(), device), m_data(NULL), m_device(device)
    {
  ...
  private:
    ...
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE static bool
    isPowerOfTwo(Index x)
    {
        eigen_assert(x > 0);
        return !(x & (x - 1));
    }
    ...
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void
    processDataLineBluestein(ComplexScalar* line_buf, Index line_len, Index good_composite, Index log_len, ComplexScalar* a, ComplexScalar* b, const ComplexScalar* pos_j_base_powered)
    {
        Index n = line_len;
    ...
    template <int Dir>
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void
    butterfly_2(ComplexScalar* data)
    {
    ...
    template <int Dir>
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void
    butterfly_4(ComplexScalar* data)
    {
        ComplexScalar tmp[4];
        tmp[0] = data[0] + data[1];
        tmp[1] = data[0] - data[1];
    ...
    template <int Dir>
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void
    butterfly_8(ComplexScalar* data)
    {
