...
struct sum_op
{
    template<typename A, typename B> EIGEN_DEVICE_FUNC constexpr static inline auto
...
{
    template<typename A, typename B> constexpr static inline auto
    run(A a, B b) -> decltype(a && b)
    {
        return a && b;
    }
...
struct not_op
{
    template<typename A> constexpr static inline auto
    run(A a) -> decltype(!a)
    {
        return !a;
    }
...
{
    EIGEN_DEVICE_FUNC constexpr static inline auto
    run(array<T, N> arr, T identity) -> decltype(Reducer::run(h_array_reduce<Reducer, T, N, n - 1>::run(arr, identity), array_get<n>(arr)))
    {
...
template<typename Reducer, typename T, std::size_t N>
EIGEN_DEVICE_FUNC constexpr inline auto
...
h_array_zip_and_reduce(array<A, N> a, array<B, N> b, numeric_list<int, n...>) -> decltype(reduce<Reducer, typename id_numeric<int,n,decltype(Op::run(A(), B()))>::type...>::run(Op::run(array_get<n>(a), array_get<n>(b))...))
{
...
constexpr inline auto
array_zip_and_reduce(array<A, N> a, array<B, N> b) -> decltype(h_array_zip_and_reduce<Reducer, Op, A, B, N>(a, b, typename gen_numeric_list<int, N>::type()))
{
...
constexpr inline auto
h_array_apply_and_reduce(array<A, N> arr, numeric_list<int, n...>) -> decltype(reduce<Reducer, typename id_numeric<int,n,decltype(Op::run(A()))>::type...>::run(Op::run(array_get<n>(arr))...))
{
...
constexpr inline auto
array_apply_and_reduce(array<A, N> a) -> decltype(h_array_apply_and_reduce<Reducer, Op, A, N>(a, typename gen_numeric_list<int, N>::type()))
{
...
struct h_repeat
{
    ...
    constexpr static inline array<t, n>
