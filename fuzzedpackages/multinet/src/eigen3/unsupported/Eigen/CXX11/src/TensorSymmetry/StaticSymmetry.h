...
struct tensor_static_symgroup_do_apply<internal::type_list<first, next...>>
{
    ...
    static inline RV
    run(const std::array<Index, NumIndices>& idx, RV initial, Args&&... args)
    {
    ...
    static inline RV
    run(const std::vector<Index>& idx, RV initial, Args&&... args)
    {
...
struct tensor_static_symgroup_do_apply<internal::type_list<EIGEN_TPL_PP_SPEC_HACK_USE(empty)>>
{
    template<typename Op, typename RV, std::size_t SGNumIndices, typename Index, std::size_t NumIndices, typename... Args>
    static inline RV
    run(const std::array<Index, NumIndices>&, RV initial, Args&&...)
    {
        // do nothing
        return initial;
    }
    ...
    template<typename Op, typename RV, std::size_t SGNumIndices, typename Index, typename... Args>
    static inline RV
    run(const std::vector<Index>&, RV initial, Args&&...)
    {
        // do nothing
        return initial;
    }
...
class StaticSGroup
{
  ...
  public:
    ...
    StaticSGroup() {}
    constexpr inline
    StaticSGroup(const StaticSGroup<Gen...>&) {}
    constexpr inline
    StaticSGroup(StaticSGroup<Gen...>&&) {}

    template<typename Op, typename RV, typename Index, std::size_t N, typename... Args>
    static inline RV
    apply(const std::array<Index, N>& idx, RV initial, Args&&... args)
    {
    ...
    static inline RV
    apply(const std::vector<Index>& idx, RV initial, Args&&... args)
    {
