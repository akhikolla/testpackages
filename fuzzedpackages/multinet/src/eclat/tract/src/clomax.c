...
cmt_create (MEMSYS *mem, int dir, ITEM size)
{
    ...
    if (!cmt)
    {
        return NULL;    /* create a c/m prefix tree */
    }
...
int
cmt_add (CMTREE *cmt, const ITEM *items, ITEM n, RSUPP supp)
{
    ...
    do                            /* traverse the items of the set */
    {
        ...
        if (--n < 0)
        {
            return 0;    /* if all items are processed, abort */
        }

        i = *items++;               /* get the next item in the set and */
        p = &node->children;        /* traverse the list of children */

        if (cmt->dir < 0) while (*p && ((*p)->item > i))
            {
                p = &(*p)->sibling;
            }

        else              while (*p && ((*p)->item < i))
            {
                p = &(*p)->sibling;
            }
    ...
    if (!node)
    {
        return -1;    /* create a new prefix tree node */
    }
...
static CMNODE*
copy (const CMNODE *src, MEMSYS *mem)
{
    /* --- copy a c/m subtree */
    ...
    do                            /* sibling copying loop */
    {
        *end = node = (CMNODE*)ms_alloc(mem);
        ...
        if (!node)
        {
            return NULL;    /* create a copy for each node */
        }
...
static void*
delclr (CMTREE *cmt, int del)
{
    /* --- delete or clear a prefix tree */
    if (del)
    {
        cmt_delete(cmt, 1);    /* if deletion flag is set, delete, */
    }

    else
    {
        cmt_clear (cmt);    /* otherwise only clear the tree */
    }
...
cmt_project (CMTREE *dst, CMTREE *src, ITEM item)
{
    ...
    if (!dst)
    {
        dst = cmt_create(NULL, src->dir, src->size-1);
    }

    if (!dst)
    {
        return NULL;    /* create/reinit. destination tree */
    }
                   ...
                   const ITEM *keep, ITEM n)
{
    ...
    if (!dst)
    {
        return NULL;    /* create the destination tree */
    }
...
static void
show (CMNODE *node, ITEMBASE *base, int ind)
{
    ...
    while (node)                  /* traverse the node list */
    {
        ...
        if (base)
        {
            printf("%s/", ib_xname(base, node->item));
        }
...
cm_create (int dir, ITEM size)
{
    ...
    if (!cm)
    {
        return NULL;    /* create a closed/maximal filter */
    }
...
cm_add (CLOMAX *cm, ITEM item, RSUPP supp)
{
    ...
    if (!*p || !cmt_valid(*p))    /* if there is no (valid) tree, */
    {
        ...
        if (!t)
        {
            return -1;    /* project it to the corresponding */
        }
    ...
    if (t->max >= supp)
    {
        return 0;    /* check the support of the prefix */
    }

    ++cm->cnt;
    return 1;          /* count the added item*/
}  /* cm_add() */
...
cm_tail (CLOMAX *cm, const ITEM *items, ITEM n)
{
    ...
    if (n == 0)
    {
        return 1;    /* empty tails can be ignored */
    }

    if (cm->cnt <= 0)
    {
        return 0;    /* check for a non-empty prefix */
    }
    ...
    if (!t)
    {
        return -1;    /* project parent with the tail items */
    }
    ...
    *p = t;                       /* set the created projection */
    ...
    if (n < 0)
    {
        return 0;    /* if not to check support, abort */
    }
...
int
cm_update (CLOMAX *cm, const ITEM *items, ITEM n, RSUPP supp)
{
