...
m16_create (int dir, SUPP supp, ISREPORT *report)
{
    ...
    if (!fim)
    {
        return NULL;    /* allocate the base structure */
    }
...
m16_add (FIM16 *fim, BITTA tract, SUPP wgt)
{
    ...
    if (tract == 0)
    {
        return;    /* ignore empty transactions */
    }
...
void
m16_addx (FIM16 *fim, const ITEM *items, ITEM n, SUPP wgt)
{
    /* --- add a transaction */
    ...
    for (tract = 0; --n >= 0; items++)
    {
        if      (*items <  0)
        {
            tract = (BITTA)(tract | (BITTA)*items);
        }

        else if (*items < 16)
        {
            tract = (BITTA)(tract | (BITTA)(1 << *items));
        }
...
static void
project (FIM16 *fim, ITEM item, BITTA mask, FIM16 *dst)
{
...
count (FIM16 *fim, int n)
{
    ...
    while (--n > 1)               /* traverse the (lower) items */
    {
        ...
        for (s = 0; t < e; t++)     /* traverse the item's transactions */
        {
            ...
            if (!p)
            {
                continue;    /* if projection is empty, skip it */
            }
...
static BITTA
filter (FIM16 *fim, int n, SUPP pex)
{
...
static int
rec_pos (FIM16 *fim, int n, BITTA mask)
{
            ...
            &&  (fim->supps[0] >= fim->smin))
    {
        ...
        if (r < 0)
        {
            return -1;    /* add item 0 to the reporter */
        }
            ...
            &&  (fim->supps[1] >= fim->smin))
    {
        ...
        if (r < 0)
        {
            return -1;    /* add item 1 to the reporter */
        }
            ...
            &&  (fim->supps[2] >= fim->smin))
    {
        ...
        if (r < 0)
        {
            return -1;    /* add item 2 to the reporter */
        }
        ...
        if (r > 0)                  /* if  item 2 needs processing, */
        {
            ...
            if      (s >= fim->supps[2])
            {
                isr_addpex(fim->report, fim->map[1]);
            }
            ...
            if (r < 0)
            {
                return -1;    /* find freq. item sets recursively */
            }
            ...
            &&  (fim->supps[3] >= fim->smin))
    {
        ...
        if (r < 0)
        {
            return -1;    /* add item 3 to the reporter */
        }
        ...
        if (r > 0)                  /* if  item 3 needs processing, */
        {
            ...
            if      (s >= fim->supps[3])
            {
                isr_addpex(fim->report, fim->map[2]);
            }
    ...
    for (i = 4; i < n; i++)       /* traverse the (remaining) items */
    {
        ...
        if (s < fim->smin)
        {
            continue;    /* skip infrequent/eliminated items */
        }
        ...
        if (r <  0)
        {
            return -1;    /* add item i to the reporter */
        }
        ...
        if (r <= 0)                 /* check if item i needs processing, */
        {
            clear(fim, i);
            continue;
        } /* otherwise delete its transactions */
        ...
        if (fim->ends[i]-fim->btas[i] <= 1)  /* if only one transaction, */
        {
            ...
            for (r = 0; (unsigned int)(1 << r) <= (unsigned int)m; r++)
                if (m & (1 << r))
                {
                    isr_addpex(fim->report, fim->map[r]);
                }
        }
...
static int
rec_neg (FIM16 *fim, int n, BITTA mask)
{
    /* --- find frequent item sets */
    int   i, r;                   /* loop variable, error status */
    SUPP  s;                      /* item support */
...
m16_clear (FIM16 *fim)
{
    ...
    for (s = 16; --n >= 0; fim++, s--)
    {
        if (fim->ttw <= 0)
        {
            continue;    /* skip empty sub-machines */
        }

        fim->ttw = 0;
        fim->tor = 0; /* clear the total trans. weight */
...
m16_mine (FIM16 *fim)
{
    ...
    if (fim->ttw <= 0)
    {
        return 0;    /* if the machine is empty, abort */
    }
    ...
    if (fim->wgts[m] >= s)        /* if there is only one transaction, */
    {
        ...
        i = hibit[m];
        fim->ends[i] = fim->btas[i];
        fim->ttw = 0;
        fim->tor = 0; /* clear 16 items machine */
    ...
    fim->ttw = 0;
    fim->tor = 0;   /* clear the total transaction weight */
...
int main (int argc, char *argv[])
{
    ...
    if (supp > 100)
    {
        error(E_SUPPORT, supp);
    }

    if (repeat < 1)
    {
        error(E_REPEAT, repeat);
    }
    ...
    if (k)
    {
        error(k, isr_name(report));    /* open the output file */
    }
    ...
    if (!fim16)
    {
        error(E_NOMEM);    /* create a 16 items machine */
    }
    ...
    for (i = 0; i < repeat; i++)  /* repeated mining loop */
    {
        ...
        if (k < 0)
        {
            error(E_NOMEM);    /* with 16 items machine */
        }
    }
