...
class MTRand
{
  ...
  public:
    ...
    double
    rand();                          // real number in [0,1]
    double
    rand( const double& n );         // real number in [0,n]
    double
    randExc();                       // real number in [0,1)
    double
    randExc( const double& n );      // real number in [0,n)
    double
    randDblExc();                    // real number in (0,1)
    double
    randDblExc( const double& n );   // real number in (0,n)
    uint32
    randInt();                       // integer in [0,2^32-1]
    uint32
    ...
    void
    load( uint32 *const loadArray );  // from such array
    friend std::ostream&
    operator<<( std::ostream& os, const MTRand& mtrand );
    friend std::istream&
  ...
  protected:
    ...
    reload();
    uint32
    hiBit( const uint32& u ) const
    {
        return u & 0x80000000U;
    }
    uint32
    loBit( const uint32& u ) const
    {
        return u & 0x00000001U;
    }
    uint32
    loBits( const uint32& u ) const
    {
    ...
    uint32
    twist( const uint32& m, const uint32& s0, const uint32& s1 ) const
    {
...
inline void
MTRand::seed( uint32 *const bigSeed )
{
...
inline MTRand::uint32
MTRand::hash( time_t t, clock_t c )
{
