...
Subset fastSparseLTS(const MatrixXd& x, const VectorXd& y,
		const double& lambda, const MatrixXi& initial, const bool& normalize, 
    const bool& useIntercept, const int& ncstep, int& nkeep, const double& tol, 
    const double& eps, const bool& useGram, int& ncores, double& center, 
    double& scale) {
	// initializations
