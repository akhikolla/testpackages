...
STCXEMnonspatial::STCXEMnonspatial(const S4 & input, const List & inputparam, const NumericMatrix & matT){
  ...
  m_hessian=mat(2*(m_model_p->m_K-1), 2*(m_model_p->m_K-1), fill::zeros); 
  m_Mjte= cube(m_data_p->m_JJ, m_data_p->m_TT, 2, fill::ones); 
  m_Mjte.slice(1).each_row() =  trans(m_data_p->m_m);
}




double STCXEMnonspatial::ComputeLogLike(){
  m_tig.zeros();
  ...
  for (int g=0; g<m_model_p->m_G;g++){
    ...
    for (int t=0; t<m_data_p->m_TT; t++){
      m_poidspolynom = m_paramCurrent_p->m_lambda[g].col(0) + m_data_p->m_m(t) * m_paramCurrent_p->m_lambda[g].col(1);
...
void STCXEMnonspatial::NewtonLogitWeighted(const int g){
  ...
  Col<double> gradient = vec(2*(m_model_p->m_K-1), fill::zeros) ;
  ...
  for (int t=0; t< m_data_p->m_TT; t++){
    Mat<double> tmp = mat(m_data_p->m_JJ, m_model_p->m_K, fill::zeros); 
    tmp.each_row() +=  trans(m_paramCurrent_p->m_lambda[g].col(0)) + trans(m_paramCurrent_p->m_lambda[g].col(1) * m_data_p->m_m(t));
    ...
    for (int k2=k1; k2< (m_model_p->m_K-1); k2++){
      for (int h1=0; h1 < 2; h1++){
        for (int h2=0; h2 < 2; h2++){
          m_hessian(k1*2 + h1, k2*2 + h2) = accu(totalsite % m_Mjte.slice(h1) % m_Mjte.slice(h2) % ratioexp.slice(k1+1) % ratioexp.slice(k2+1));
          m_hessian(k2*2 + h2, k1*2 + h1) = m_hessian(k1*2 + h1, k2*2 + h2);
        }
      }
    }
    for (int h1=0; h1 < 2; h1++){
      for (int h2=h1; h2 < 2; h2++){
        m_hessian(k1*2 + h1, k1*2 + h2) -= accu(totalsite % m_Mjte.slice(h1) % m_Mjte.slice(h2) % ratioexp.slice(k1+1));
        m_hessian(k1*2 + h2, k1*2 + h1) = m_hessian(k1*2 + h1, k1*2 + h2);
      }
      gradient(k1*2+h1) = accu((u.slice(k1+1) - totalsite % ratioexp.slice(k1+1)) % m_Mjte.slice(h1) );
    }
  }   
  ...
  if (m_nondegeneracy == 1){
    ...
    for (int k=0; k< (m_model_p->m_K-1); k++) m_paramCurrent_p->m_lambda[g].row(k+1) = trans(tmp.subvec(k*2, (k*2)+1));
  }
}

void STCXEMnonspatial::Mstep(){
