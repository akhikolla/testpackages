...
cmaes_UpdateEigensystem(cmaes_t *t, int flgforce)
{
  ...
  if(flgforce == 0) {
    ...
    if (t->sp.updateCmode.maxtime < 1.00
        && 0 // DANIEL: Added that to disable the op
...
random_init( random_t *t, long unsigned inseed)
{
  ...
  if (inseed < 1) {
    ...
    inseed = (long unsigned) abs( (long int) (100*time(NULL)+clock()));
  }
...
readpara_SupplementDefaults(readpara_t *t)
{
  ...
  if (t->seed < 1) {
    while ((int) (cloc - clock()) == 0)
      ; /* TODO: remove this for time critical applications!? */
    t->seed = (unsigned int) abs( (long int) (100*time(NULL)+clock()) );
  }
...
void cmaes_print_info(cmaes_t *t) {
    ...
    //fprintf(fp, "function evaluations %.0f\n", t->countevals);
    //fprintf(fp, " seed %d\n", t->sp.seed);
    //fprintf(fp, " aktseed %d\n", t->rand.aktseed);
    //fprintf(fp, " aktrand %d\n", t->rand.aktrand);
    //fprintf(fp, " flgstored %d\n", t->rand.flgstored);
    //fprintf(fp, " hold %f\n", t->rand.hold);
}
