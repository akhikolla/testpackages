...
#define WANT_STREAM                    // includenew.h will get stream fns
#define WANT_STRING

#include "includenew.h"                   // include standard files
#include "debug.h"
...
Janitor::~Janitor()
{
   ...
   {
#ifdef CLEAN_LIST
      print_info("Delete from  clean-list %lu\n", (unsigned long)this);
#endif
...
{ print_info("   %s %lu\n", ClassName (unsigned long)ClassStore); }

void FCLRealArray::Report()
{

{ print_info("   %s %lu %d\n", Operation, (unsigned long)ClassStore, size); }
...
void FreeCheck::Register(void* t, char* name)
{
   ...
   if (!f) { print_error("Out of memory in FreeCheck\n"); exit(1); }
#ifdef REG_DEREG
   print_info("Registering   %s  %lu\n", name, (unsigned long)t);
#endif
}
...
void FreeCheck::Status()
{
   ...
   if (BadDelete)
   {
      print_error("\nThere were %d request to delete non-existent items\n\n", BadDelete);
   }
}
