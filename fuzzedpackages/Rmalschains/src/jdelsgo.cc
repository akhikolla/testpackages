...
JDE::JDE(Random *random, int reduced) : ClassEAlgorithm(random) {
...
unsigned JDE::realApply(tChromosomeReal &sol, tFitness &fitness) {
    const int D=m_problem->getDimension();
    tChromosomeReal crom(D);
    tChromosomeReal best(D);
    tIndividualRealJDE* nov;
   ...
   for (it = 0; !m_running->isFinish(); it++) {
	...
	string strategy = getStrategy(NP, it, MaxFES);
...
string JDE::getStrategy(unsigned NP, unsigned iteration, unsigned MaxFES) {
    string strategy;

    if (m_random->rand() < 0.1 && iteration > MaxFES/2)
      strategy = "jDEbest";
    else if (iteration < (NP/2) )
      strategy = "jDEbin";
    else
      strategy = "jDEexp";
...
void JDE::jDE(const int i, const int NP, tIndividualRealJDE* &newsol, tChromosomeReal &best, string strategy, int it, int MaxFES) {
     int r1, r2, r3;
     double tau1 = 0.1;
     double tau2 = 0.1;
     double F, CR;
     ...
     tIndividualRealJDE *ind = (tIndividualRealJDE *) m_pop->getInd(i);
    ...
    // border check
     ...
     newsol = (tIndividualRealJDE*) m_pop->getInstance(tmp);
     newsol->setF(strategy, F);
     newsol->setCR(strategy, CR);
} // end jDE   


int JDE::mRandomInt(unsigned size) {
...
void JDE::chooseRs(int LB, int UB, int NP, int i, int &r1, int &r2, int& r3) {
   int size = UB - LB; // 
   do { 
 ...
 */
void JDE::setPopsize(unsigned int popsize) {
    if (m_pop) {
	delete m_pop;
    }

    m_pop = new PopulationRealJDE(m_random,popsize, popsize);
}
