    ...
    arma::rowvec inner_loop(arma::mat& MATRIX_1st, arma::mat& MATRIX_2nd, int i, int ITERS, int k, std::string& method, arma::mat& cov_mat, double eps = 1.0e-6) {
      ...
      for (int j = 0; j < ITERS; j++) {                                                                                                                                                        // http://scikit-learn.org/stable/modules/generated/sklearn.neighbors.DistanceMetric.html
       ...
       // else if (method == "cosine") {
         ...
         // tmp_idx = arma::as_scalar(1.0 - arma::accu(MATRIX_1st.row(j) * MATRIX_2nd.row(i)) / (std::sqrt(arma::accu(arma::square(MATRIX_1st.row(j) + eps))) * std::sqrt(arma::accu(arma::square(MATRIX_2nd.row(i) + eps)))));
       //}

        else if (method == "schi") {

          tmp_idx = arma::as_scalar(arma::accu(arma::square(MATRIX_1st.row(j) - MATRIX_2nd.row(i) + eps) /(MATRIX_1st.row(j) + MATRIX_2nd.row(i) + eps)));
