...
return(list("starting.parameters" = fs.results[,1:ar.order.max],
            "starting.rejection.probs"     = - fs.results[,ar.order.max+1],
            "first.stage.parameters" = fs.results[,
                                       (ar.order.max+2):(2*ar.order.max + 1)]*margin,
            "first.stage.rejection.probs" = - fs.results[,2*ar.order.max+2],
            "second.stage.parameters" = fs.results[index.top,
                                 (2*ar.order.max+3):(3*ar.order.max + 2)]*margin, 
            "second.stage.rejection.probs" = - fs.results[index.top,3*ar.order.max+3],
            "convergence" = fs.results[index.top,3*ar.order.max+4],
            "size" = max(- fs.results[index.top, 3*ar.order.max+3])
            ))
}