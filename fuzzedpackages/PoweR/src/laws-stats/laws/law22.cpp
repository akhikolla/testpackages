...
extern "C" {
  ...
  void law22 (int *xlen, double *x, char **name, int *getname, double *params, int *nbparams, int *setseed) {

    int i, j = 0, n = xlen[0];
    if (getname[0] == 1) {
// Here, INDICATE the name of the distribution:
      const char *nom = "$ScConN(p,d)$";
...
// If necessary, we check if some parameter values are out of parameter space
    if (d <= 0.0 || p < 0.0 || p > 1.0) {
      warning("d should be >0 and p should be in [0,1] in law22!\n");
...
// Generation of the random values
    ...
    for (i = 0; i < n;i++)   {
      if (U[i] < p) x[i] = Rf_rnorm(0.0, d); else x[i] = Rf_rnorm(0.0, 1.0);
    }
