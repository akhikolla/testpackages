// Title: Cauchy(mu,sigma)
// Ref. (book or article): See function rcauchy() from package stats
...
extern "C" {
  ...
  void law3 (int *xlen, double *x, char **name, int *getname, double *params, int *nbparams, int *setseed) {
    
    int i, j = 0, n = xlen[0];
    if (getname[0] == 1) {
// Here, INDICATE the name of the distribution:
      const char *nom = "$Cauchy(\\mu,\\sigma)$";
...
// Generation of the random values
    ...
    for (i = 0; i < n; i++) x[i] = Rf_rcauchy(mu, sigma);
