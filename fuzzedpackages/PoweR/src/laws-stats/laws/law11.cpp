// Title: Weibull(shape,scale)
// Ref. (book or article): See function rweibull from package stats()
...
extern "C" {
  ...
  void law11 (int *xlen, double *x, char **name, int *getname, double *params, int *nbparams, int *setseed) {

    int i, j = 0, n = xlen[0];
    if (getname[0] == 1) {
// Here, INDICATE the name of the distribution:
      const char *nom = "$Weibull(shape,scale)$";
...
// Generation of the random values
    ...
    for (i = 0; i < n; i++) x[i] = Rf_rweibull(shape,scale);
