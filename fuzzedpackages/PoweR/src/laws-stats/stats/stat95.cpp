...
extern "C" {
  ...
  void stat95(double *x, int *xlen, double *level, int *nblevel, char **name, int *getname, double *statistic, int *pvalcomp, double *pvalue, double *critvalL, double *critvalR, int *usecrit, int *alter, int *decision, double *paramstat, int *nbparamstat) {
...
// 4: bilateral test that rejects H0 only for small values of the test statistic
    ...
    if (n > 3) {
      ...
      double statHoV, muhat, tmp = 0.0, bhat;
	...
	tmp = tmp + fabs(x[i] - muhat);
      ...
      statHoV = 0.5 * (x[n - 1] - x[0]) / bhat;

      statistic[0] = statHoV; // Here is the test statistic value

      if (pvalcomp[0] == 1) {
	// If possible, computation of the p-value.
#include "pvalues/pvalue95.cpp"
      }
