...
extern "C" {
  ...
  void stat45(double *x, int *xlen, double *level, int *nblevel, char **name, int *getname, double *statistic, int *pvalcomp, double *pvalue, double *critvalL, double *critvalR, int *usecrit, int *alter, int *decision, double *paramstat, int *nbparamstat) {
...
// 4: bilateral test that rejects H0 only for small values of the test statistic
	...
	KSa[i] = plaplace((x[i] - muhat) / bhat);
      ...
      // calculate statKS
      Dplus = 1.0 / (double)n - KSa[0];
      Dminus = KSa[0];
      for (i = 1; i < n; i++) {
	tmp2 = (double)(i + 1) / (double)n - KSa[i];
	if (tmp2 > Dplus) Dplus = tmp2;
	tmp2 = KSa[i] - ((double)i) / (double)n;
	if (tmp2 > Dminus) Dminus = tmp2;
      }
      statKS = Dplus;
      if (Dminus > statKS) statKS = Dminus;
      
      statKS = sqrt((double)n) * statKS;	
