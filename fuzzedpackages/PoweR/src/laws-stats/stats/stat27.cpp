...
// Computation of the value of the test statistic
    ...
    b = new double[n];
    xs = new double[n];
    double statQ, q1=0.0, q2=0.0, term=0.0;
    ...
    for (i=1;i<=n;i++) {
	  u[i-1] = qnorm(((double)i-0.375)/((double)n+0.25),0.0,1.0,1,0);
	}
	...
	// b[n-4] = 1.0/(((double)(n-4))*(u[n-4]-u[n-8]));
	
	// b[n-3] = 1.0/(((double)(n-4))*(u[n-3]-u[n-7]));
	
	// b[n-2] = 1.0/(((double)(n-4))*(u[n-2]-u[n-6]));
	
	// b[n-1] = 1.0/(((double)(n-4))*(u[n-1]-u[n-5]));
    ...
    statistic[0] = statQ; // Here is the test statistic value


if (pvalcomp[0] == 1) {
	// If possible, computation of the p-value.
	#include "pvalues/pvalue27.cpp"
}
