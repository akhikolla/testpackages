...
extern "C" {
  ...
  void stat82(double *x, int *xlen, double *level, int *nblevel, char **name, int *getname, double *statistic, int *pvalcomp, double *pvalue, double *critvalL, double *critvalR, int *usecrit, int *alter, int *decision, double *paramstat, int *nbparamstat) {
...
// Computation of the value of the test statistic
	...
	for (i=0; i<n; i++) {
	  ...
	  sumZC = sumZC + R_pow(log((1.0/U[i]-1.0)/(((double)n-1.0/2.0)/((double)i+1.0-3.0/4.0)-1.0)),2.0);
