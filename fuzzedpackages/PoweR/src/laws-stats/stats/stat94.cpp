...
extern "C" {
  ...
  void stat94(double *x, int *xlen, double *level, int *nblevel, char **name, int *getname, double *statistic, int *pvalcomp, double *pvalue, double *critvalL, double *critvalR, int *usecrit, int *alter, int *decision, double *paramstat, int *nbparamstat) {
...
// 4: bilateral test that rejects H0 only for small values of the test statistic
    ...
    if (n > 3) {
      ...
      double xbar = 0.0, S2 = 0.0, statHoU, muhat, tmp = 0.0, bhat;
      // calculate xbar
      for (i = 0; i < n; i++) {
	xbar = xbar + x[i];
      }
	...
	tmp = tmp + fabs(x[i] - muhat);
      ...
      statHoU = sqrt(S2) / bhat;

      statistic[0] = statHoU; // Here is the test statistic value

      if (pvalcomp[0] == 1) {
	// If possible, computation of the p-value.
#include "pvalues/pvalue94.cpp"
      }
