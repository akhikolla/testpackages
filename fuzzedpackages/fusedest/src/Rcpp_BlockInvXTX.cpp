...
Eigen::SparseMatrix<double> ComputeBlockInvXTX(Eigen::MatrixXd X_s,
                                               SEXP a,
                                               SEXP ind_strt,
                                               SEXP n_dc){
  ...
  MatrixXd inv_XTX_j = MatrixXd(p, p).setZero();
  ...
  for(int j = 0; j < m; j++){
    ...
    inv_XTX_j = XTX_j.llt().solve(Id);

    for(int k = 0; k < p; k++){
      for(int l = 0; l < p; l++){
                   tripletList.push_back(T(p*j + k, p*j + l, inv_XTX_j(k, l)));
                                        }
                                      }
    XTX_j.setZero();
    inv_XTX_j.setZero();
  ...
  Eigen::SparseMatrix<double> inv_XTX_plus_a(m*p, m*p);

  inv_XTX_plus_a.setFromTriplets(tripletList.begin(), tripletList.end());

  return inv_XTX_plus_a;
                           }
