opt82 <- eventReactive(input$RunOpt82, {
    Gprint(MODE_DEBUG, "Opt82\n")
    ficout = tempfile()
    ...
    if (is.null(ficin)) {
        ...
        show("spinner")
        tryCatch(diploidize(ficin, outputFile = ficout), error = function(e) {
            file.create(ficout)
...
output$downloadOpt82All <- downloadHandler(filename = function() {
    paste("result_opt82_", Sys.Date(), ".txt", sep = "")
