opt73 <- eventReactive(input$RunOpt73, {
    Gprint(MODE_DEBUG, "Opt73\n")
    ficout = tempfile()
    ...
    if (is.null(ficin)) {
        ...
        show("spinner")
        tryCatch(conversion(ficin, format = "BiosysN", outputFile = ficout), error = function(e) {
            file.create(ficout)
...
output$downloadOpt73All <- downloadHandler(filename = function() {
    paste("result_opt73_", Sys.Date(), ".txt", sep = "")
