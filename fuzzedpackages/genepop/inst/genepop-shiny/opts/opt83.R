opt83 <- eventReactive(input$RunOpt83, {
    Gprint(MODE_DEBUG, "Opt83\n")
    ficout = tempfile()
    ...
    if (is.null(ficin)) {
        ...
        show("spinner")
        tryCatch(relabel_alleles(ficin, outputFile = ficout), error = function(e) {
            file.create(ficout)
...
output$downloadOpt83All <- downloadHandler(filename = function() {
    paste("result_opt83_", Sys.Date(), ".txt", sep = "")
