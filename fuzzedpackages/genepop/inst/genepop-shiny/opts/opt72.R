opt72 <- eventReactive(input$RunOpt72, {
    Gprint(MODE_DEBUG, "Opt72\n")
    ficout = tempfile()
    ...
    if (is.null(ficin)) {
        ...
        show("spinner")
        tryCatch(conversion(ficin, format = "BiosysL", outputFile = ficout), error = function(e) {
            file.create(ficout)
...
output$downloadOpt72All <- downloadHandler(filename = function() {
    paste("result_opt72_", Sys.Date(), ".txt", sep = "")
