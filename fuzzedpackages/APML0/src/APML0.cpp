...
List cvTrimLmC(Eigen::VectorXd beta, int nn, int nn2, Eigen::VectorXi loco,
               Eigen::MatrixXd X, Eigen::VectorXd y, int N, int p,
               Eigen::MatrixXd XF, Eigen::VectorXd yF, int NF) {
  int i, j;
  Eigen::VectorXd mX(p);
  Eigen::VectorXd RSS, RSSp, xbF=Eigen::VectorXd::Zero(NF), xb=Eigen::VectorXd::Zero(N);
  double my, a0;
  ...
  if (nn==0) {
   ...
   RSS(0)=(y-xb).squaredNorm();
   RSSp(0)=(yF-xbF).squaredNorm();
    ...
    for (i=0; i<nn; ++i) {
      ...
      RSS(i)=(y-xb).squaredNorm();
      RSSp(i)=(yF-xbF).squaredNorm();
...
List EnetLmC(Eigen::MatrixXd X, Eigen::VectorXd y,
  ...
  exit:
  return(List::create(Named("Beta")=Beta, Named("BetaSTD")=BetaSTD, Named("a")=a0S, Named("flag")=flag, Named("rsq")=RSQ,
...
List NetLmC(Eigen::MatrixXd & X, Eigen::VectorXd & y, double alpha,
            Eigen::VectorXd lambda, int nlambda, int ilambda, Eigen::VectorXd wbeta, Eigen::VectorXd wbeta2,
            Eigen::SparseMatrix<double> & Omega, Eigen::MatrixXd loc, Eigen::VectorXi nadj,
            int p, int N0, double thresh, int maxit, double thresh2){

  int i, j, ij, m, it=0, il, iadd, ia=0;
  double lambda2, lambda2i, zi, zi2, objQi=0.0, objQj, obj0, obj1, rss0, rss1, b0, db0, RSS0;
  Eigen::VectorXd beta0=Eigen::VectorXd::Zero(p);
  Eigen::MatrixXd Beta=Eigen::MatrixXd::Zero(p,nlambda),BetaSTD=Eigen::MatrixXd::Zero(p,nlambda);
  Eigen::VectorXd lambda1(p);
  Eigen::VectorXi active=Eigen::VectorXi::Zero(p), iactive=Eigen::VectorXi::Zero(p);
      ...
      while(1){
        ...
        for(i=0;i<ia;++i){
          ...
          if(zi>lambda1(j)){
            b0=(zi-lambda1(j))/(lambda2i+1.0);
            db0=beta0(j)-b0;
            objQi-=db0*(beta0(j)+b0-2.0*zi2)*lambda2i; //  beta^T*L*beta
            beta0(j)=b0;
            y+=db0*X.col(j);
            objQj+=std::abs(b0)*lambda1(j);
          }else if(zi<-lambda1(j)){
            b0=(zi+lambda1(j))/(lambda2i+1.0);
            db0=beta0(j)-b0;
            objQi-=db0*(beta0(j)+b0-2.0*zi2)*lambda2i;
            beta0(j)=b0;
            y+=db0*X.col(j);
            objQj+=std::abs(b0)*lambda1(j);
          }else{
...
List cvNetLmC(Eigen::MatrixXd & X, Eigen::VectorXd & y,double alpha,
              Eigen::VectorXd lambda, int nlambda, Eigen::VectorXd wbeta, Eigen::VectorXd wbeta2,
              Eigen::SparseMatrix<double> & Omega, Eigen::MatrixXd loc, Eigen::VectorXi nadj,
              int N, int p, double thresh, int maxit, Eigen::MatrixXd XF, Eigen::VectorXd yF, int NF, double thresh2){
...
/*****  Cox: Used for CV trimming of number  *****/
// [[Rcpp::export]]
...
/*****  Cox: Used for CV hard-thresholding cut Beta *****/
// [[Rcpp::export]]
Eigen::VectorXd cvHardCoxC(Eigen::VectorXd beta, Eigen::VectorXd betaSTD, Eigen::VectorXd cut, Eigen::VectorXd wbeta, int p,
						   Eigen::MatrixXd XF, int NF,
                           Eigen::VectorXi neventF, Eigen::VectorXi nevent1F, Eigen::VectorXi loc1F, int nF,
                           Eigen::MatrixXd X, int N,
                           Eigen::VectorXi nevent, Eigen::VectorXi nevent1, Eigen::VectorXi loc1, int n,
  ...
  int i, j, nc=cut.size();
  double lli, lfi;
  Eigen::VectorXd betaC(p);
  Eigen::VectorXd cv(nc), xbF=Eigen::VectorXd::Zero(NF), xb=Eigen::VectorXd::Zero(N);
  Eigen::VectorXd exb(N), exbF(NF);
...
List EnetCoxC(Eigen::MatrixXd X, Eigen::VectorXd tevent,
  ...
  double lambda2, lambda2i, lambda2j, zi, obj0, obj1, ll0, ll1, b0, db0, PLi, PLi2, objQi, objQj;
  Eigen::VectorXd beta0=Eigen::VectorXd::Zero(p);
    ...
    local:
      while(1){
        ...
        for(i=0;i<ia;++i){
		  ...
		  lambda2j=lambda2i*wbeta2(j);
          if(zi>lambda1i(j)){
            b0=(zi-lambda1i(j))/(lambda2j+PLi2);
            db0=beta0(j)-b0;beta0(j)=b0;
            pl1+=(pl2.cwiseProduct(X.col(j)))*db0;
            xb-=db0*X.col(j);
            objQj+=std::abs(b0)*lambda1(j);
            objQi+=pow(b0, 2)*lambda2j;
...
List NetCoxC(Eigen::MatrixXd & X, Eigen::VectorXd tevent, double alpha,
  ...
  double zi, zi2, objQi=0.0, objQj, obj0, obj1, ll0, ll1, b0, db0, PLi, PLi2;
  Eigen::VectorXd beta0=Eigen::VectorXd::Zero(p);
  Eigen::MatrixXd Betas=Eigen::MatrixXd::Zero(p, nlambda), BetasSTD=Eigen::MatrixXd::Zero(p, nlambda);
  Eigen::VectorXd lambda1(p), lambda1i(p), lambda2(p), lambda2i(p), locbeta(nlambda);
  Eigen::VectorXi active=Eigen::VectorXi::Zero(p), iactive=Eigen::VectorXi::Zero(p);
  Eigen::VectorXi flag=Eigen::VectorXi::Zero(nlambda);
  Eigen::VectorXd exb=Eigen::VectorXd::Constant(N, 1.0), xb=Eigen::VectorXd::Zero(N);
  Eigen::VectorXd pl1(N), pl2(N);
  ...
  for(il=0;il<nlambda;++il){
    lambda1=lambda(il)*alpha*wbeta; lambda2=lambda(il)*(1.0-alpha)*wbeta2;
    lambda1i=lambda1*N0; lambda2i=lambda2*N0;
    ...
    local:
      while(1){
        ...
        for(i=0;i<ia;++i){
          ...
          if(zi>lambda1i(j)){
            b0=(zi-lambda1i(j))/(lambda2i(j)+PLi2);
            db0=beta0(j)-b0;
            objQi-=db0*(beta0(j)+b0-2.0*zi2)*lambda2i(j);
            beta0(j)=b0;
            pl1+=(pl2.cwiseProduct(X.col(j)))*db0;
            xb-=db0*X.col(j);
            objQj+=std::abs(b0)*lambda1(j);
...
List cvNetCoxC(Eigen::MatrixXd & X, Eigen::VectorXd tevent, double alpha,
  ...
  Eigen::VectorXd beta0=Eigen::VectorXd::Zero(p);
  Eigen::MatrixXd Betas=Eigen::MatrixXd::Zero(p, nlambda), BetasSTD=Eigen::MatrixXd::Zero(p, nlambda);
  Eigen::VectorXd lambda1(p), lambda1i(p), lambda2(p), lambda2i(p), locbeta(nlambda), locbetaF(nlambda);
  Eigen::VectorXi active=Eigen::VectorXi::Zero(p), iactive=Eigen::VectorXi::Zero(p);
  Eigen::VectorXi flag=Eigen::VectorXi::Zero(nlambda);
  Eigen::VectorXd exb=Eigen::VectorXd::Constant(N, 1.0), xb=Eigen::VectorXd::Zero(N);
  Eigen::VectorXd exbF(NF), xbF(NF);
...
Eigen::VectorXd cvTrimLogC(Eigen::VectorXd beta, int nn, int nn2, Eigen::VectorXi loco,
                            Eigen::MatrixXd X, Eigen::VectorXd y, int N,
                            Eigen::MatrixXd XF, Eigen::VectorXd yF, int NF, double thresh, int maxit, double threshP) {
  int i, j, it, i2;
  Eigen::VectorXd Dev(nn2), xb=Eigen::VectorXd::Zero(N), xbF=Eigen::VectorXd::Zero(NF); // xb=Eigen::VectorXd::Zero(N),
  Eigen::ArrayXd p0(N), pF0(NF);
  double beta0, beta1, db;
  ...
  for(i=0; i<nn; ++i){
    ...
    if (i>=1) {
      for (i2=1; i2<=i; ++i2) {
        j=loco(i2); //   index of nonzero beta

        xb+=X.col(j)*beta(i2);
        xbF+=XF.col(j)*beta(i2);
      }
    }


    it=0; beta1=beta(0); beta0=beta(0);
    while(1){
      for (i2=0; i2<N; ++i2) {
      ...
      beta0=beta1+(y.array()-p0).sum()/(p0*(1.0-p0)).sum();
      db=beta0-beta1;

      if(std::abs(db)<thresh){break;}
      if(it>=maxit){ break;}

      xb+=X.col(0)*db; beta1=beta0;
      ++it;
    }
    ...
    xbF+=XF.col(0)*beta0;
    for (j=0; j<NF; ++j) {
      pF0(j)=1.0/(1.0+exp(-xbF(j)));
      if (pF0(j) < threshP) {
        pF0(j)=threshP;
      } else if (pF0(j) > (1.0-threshP)) {
        pF0(j)=1.0-threshP;
      }
    }

    Dev(i)=(yF.array()*pF0.log()+(1.0-yF.array())*(1.0-pF0).log()).sum()*(-2.0);
  }
...
/*****  Used for CV hard-thresholding  *****/
// [[Rcpp::export]]
Eigen::VectorXd cvHardLogC(Eigen::VectorXd beta, Eigen::VectorXd betaSTD, Eigen::VectorXd cut, Eigen::VectorXd wbeta,
                           Eigen::MatrixXd X, Eigen::VectorXd y, int N, int p,
                           Eigen::MatrixXd XF, Eigen::VectorXd yF, int NF, double thresh, int maxit, double threshP) {
  int i, i2, j, it, nc=cut.size();
  Eigen::VectorXd betaC(p);
  Eigen::VectorXd Dev, xb=Eigen::VectorXd::Zero(N), xbF=Eigen::VectorXd::Zero(NF);
  Eigen::ArrayXd p0(N), pF0(NF);
  double beta0, beta1, db;
  ...
  for (i=0; i<nc; ++i) {
    ...
    it=0; beta1=betaC(0); beta0=betaC(0);
    while(1){
      for (i2=0; i2<N; ++i2) {
        p0(i2)=1.0/(1.0+exp(-xb(i2)));
        if (p0(i2) < threshP) {
...
List EnetLogC(Eigen::MatrixXd X, Eigen::VectorXd y,
  ...
  for(il=0;il<nlambda;++il){
    lambda1=lambda(il)*alpha*wbeta; lambda2=lambda(il)*(1.0-alpha)*wbetai; // lambda1:vector lambda*alpha, lambda2=lambda*(1-alpha)
