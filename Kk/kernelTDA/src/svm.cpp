...
public:
	...
	MatrixXd eigenvals = solver.eigenvalues();  // vector of eigenvalues
	MatrixXd eigenvecs = solver.eigenvectors(); // column matrix of eigenvectors
...
//	  l;
		...
		if((start = cache->get_data(i,&data,len)) < len)
		{
		  for(int j=start;j<len;j++)
		    data[j] = Gpos->operator()(i,j); // modified
		}
		return data;
	}
