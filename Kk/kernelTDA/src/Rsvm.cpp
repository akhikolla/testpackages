...
List svmtrain_R(Eigen::MatrixXd x, int r, int c,    
                ...
                double sigma,
                Eigen::VectorXd probA,
                Eigen::VectorXd probB,
                
                Eigen::VectorXd cresults,
                double ctotal1,
                double ctotal2) {
  
  int error_maxlen = 255;
  char *error_c = Malloc(char, error_maxlen);
  
  for (int i = 0; i < error_maxlen; i++)
    error_c[i] = '\0';
  ...
  std::string error_s(error_c);
  
  CharacterVector error = {error_s};
  
  return List::create(_["nclasses"] = nclasses, _["nr"] = nr, _["index"] = index,
                     _["labels"] = labels, _["nSV"] = nSV, _["rho"] = rho,
                     _["coefs"] = coefs, _["sigma"] = sigma, _["probA"] = probA,
                     _["cresults"] = cresults, _["ctotal1"] = ctotal1, _["ctotal2"] = ctotal2,
                     _["error"] = error);
}	     	     
...
List svmpredict_R(int decisionvalues,
                   ...
                   Eigen::MatrixXd x, int xr,
                   Eigen::VectorXi    xrowindex,
                   Eigen::VectorXi    xcolindex,
                   int sparsex,
  ...
  return List::create(_["ret"] = ret, _["dec"] = dec, _["prov"] = prob);
}
